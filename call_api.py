...
# due to API have no cert so we won"t verify SSL certificate and this is to disable warning msg
requests.packages.urllib3.disable_warnings()
fig = plt.figure()  # create figure of graph

APIs = {
    "covid_today": "https://covid19.ddc.moph.go.th/api/open/today", "covid_sum": "https://covid19.ddc.moph.go.th/api/open/cases/sum", "covid_trend": "https://covid19.ddc.moph.go.th/api/open/timeline"
}
...
def api_caller(endpoint):
    ...
    print("Calling: " + endpoint + " at time: " + calling_time +
          ", Got response code: " + resp_status_code)
    if (resp_status_code == "200"):
        resp_json = json.loads(response.content)
        print(" (" + endpoint + " data last updated on: " +
              resp_json["UpdateDate"] + ")")
        return(resp_json)
    else:
        print("Please retry as a response code is not 200, it is: " +
              resp_status_code + " at " + calling_time)

def gen_today_data(data_type):
    compare = "+" if today_resp_json["New" + data_type] >= 0 else "-"
    today_field.append(data_type + " case")
    today_value.append(str(
        today_resp_json[data_type]) + " (" + compare + str(today_resp_json["New" + data_type]) + ")")
...
def gen_covid_cases(sum_by, case_index, case_value, plot_title, top):
    sum_resp_json[sum_by].sort(
        key=lambda x: x["Count"], reverse=True)  # sort DESC
    column = []
    value = []
...
def gen_covid_trend(trend_resp_json):
    trend_resp_json["Data"].sort(key=lambda x: datetime.strptime(
        x["Date"], '%m/%d/%Y').date(), reverse=True)  # sort date DESC
    trend_data = {}
    ...
    ax = df.plot(ax=plt.gca(), lw=4, colormap="jet", marker=".",
                 markersize=5, title="Trend of COVID case in Thailand (last 90 days)")
...
gen_covid_cases("Gender", "Gender", "Count",
                "Today's COVID cases by gender", 3)
gen_covid_cases("Nation", "Nation", "Count",
                "Today's COVID cases by Nation (top 10)", 10)
gen_covid_cases("Province", "ProvinceEn", "Count",
                "Today's COVID cases by Thailand province (top 15)", 15)
...
# show graphs
plt.subplots_adjust(left=0.08, right=0.98, bottom=0.05,
                    top=0.97, wspace=0.20, hspace=0.2)
plt.show()